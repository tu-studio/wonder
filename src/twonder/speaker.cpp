...
Speaker::Speaker(float x, float y, float z, float normalx, float normaly, float normalz, float cosAlpha) {
    pos[0] = x;
    pos[1] = y;
...
void SpkArray::addSpeakers(Segment* segment) {
    ...
    for(int i = 0; i < segment->noSpeakers; ++i) {
        Speaker* spk = new Speaker(speakerPosition[0], speakerPosition[1], speakerPosition[2], segment->normal[0], segment->normal[1], segment->normal[2], cosAlpha);
        push_back(spk);
