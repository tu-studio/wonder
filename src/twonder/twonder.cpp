...
#define NUM_LISTENERS 1 // Number of tracked listeners (currently just working for one listener)
...
void jack_shutdown(void* arg) {
    std::cerr << "[JACK]: JACK server was shut down or disconnected this JACK client - exiting now!\n";
...
int process(jack_nframes_t nframes, void* arg) {
    ...
    // get the input buffers from the soundcard and put each of them into the delayline of a source
    ...
    for(unsigned int i = 0; i < speakers->size(); ++i) {
        float* output = static_cast<float*>(jack_port_get_buffer(jackOutputs[i], nframes));
        std::memset(output, 0, nframes * sizeof(jack_default_audio_sample_t));

        // calculate the delay coefficients for every source-to-speaker-to-listener combination
        for(int j = 0; j < twonderConf->noSources; ++j) {
            DelayCoeff c  = (*sources)[j]->source->getDelayCoeff(*((*speakers)[i]), *listeners);
            DelayCoeff c2 = (*sources)[j]->source->getTargetDelayCoeff(*((*speakers)[i]), nframes, *listeners);
            ...
            if((*sources)[j]->source->hasDopplerEffect()) {
                ...
                (*sources)[j]->inputline->getFadej(c, c2, output, nframes, DelayLine::dB3);
            }
        }
    }
...
bool initialize_jack() {
    ...
    const char* server_name = NULL;                  // Without a JACK server name the default server from the .jackdrc file will be started.
    jack_options_t options  = JackNullOption;        // No options needed, since we use the options from the .jackdrc file.
    jack_status_t status;                            // Holds the JACK status after the connection attempt for further verifications.
    ...
    if(jackClient == NULL) {
        running = false;
        std::cerr << "[JACK][ERROR]: Unable to open the JACK client! Failed with status: 0x" << std::hex << status << std::dec << '\n';
    ...
    if(running) {
        // Print the name of the JACK server if it was not the default one from the .jackdrc file.
        if(status & JackServerStarted) {
            std::cout << "\n[JACK]: Connection to the default JACK server established!" << std::endl;
        }

        // Print the new JACK client name if it was not unique and has been been replaced by the JACK server.
        if(status & JackNameNotUnique) {
            client_name = jack_get_client_name(jackClient);

            if(client_name == NULL) {
                std::cout << "[JACK][WARNING]: JACK client name was not unique and has been changed!" << client_name << std::endl;
            } else {
                std::cout << "[JACK][WARNING]: JACK client name was not unique! New JACK client name is: " << client_name << std::endl;
            }
        }
    }
    ...
    if(running) {
        jackInputs  = static_cast<jack_port_t**>(std::calloc(twonderConf->noSources, sizeof(jack_port_t*)));
        jackOutputs = static_cast<jack_port_t**>(std::calloc(speakers->size(), sizeof(jack_port_t*)));
    }
    ...
    if(running) {
        ...
        for(int i = 0; i < twonderConf->noSources; ++i) {
            ...
            jackInputs[i] = jack_port_register(jackClient, port_name.data(), JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);

            if(jackInputs[i] == NULL) {
                running = false;
                std::cerr << "[JACK][ERROR]: Unable to register the input ports (sources) of the JACK client!\n";
            }
        }
    }
    ...
    if(running) {
        ...
        for(int i = 0; i < static_cast<int>(speakers->size()); ++i) {
            port_name = "speaker";
            port_name.append(std::to_string(i + 1));
            jackOutputs[i] = jack_port_register(jackClient, port_name.data(), JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0);

            if(jackOutputs[i] == NULL) {
                running = false;
                std::cerr << "[JACK][ERROR]: Unable to register the output ports (speakers) of the JACK client!\n";
            }
        }
    }

    // 7. Save the JACK client samplerate to twonderConfig and initialize TimeStamp and DelayLine fade factors.
    ...
    // 8. Activate the JACK client - this starts the real-time callback by the JACK server and enables the interconnection of the ports of the JACK client.
    if(running) {
    ...
    // 9. Connect the audio inputs of the soundcard with the input ports of the JACK client (JACK calls them "output ports", because they are readable by the JACK client).
    if(running) {
        const char** ports = jack_get_ports(jackClient, NULL, NULL, JackPortIsPhysical | JackPortIsOutput);

        if(ports == NULL) {
            running = false;
            std::cerr << "[JACK][ERROR]: No physical audio input (capture) ports are available!\n";
        }

        // Proceed only if the soundcard has physical audio input (capture) ports!
        if(running) {
            for(int i = 0; i < twonderConf->noSources; ++i) {
                if(!((ports[i] == NULL) || (jack_port_name(jackInputs[i]) == NULL))) { // Do not connect to NULL ports
                    // jack_connect(jackClient, ports[i], jack_port_name(jackInputs[i])); // TODO: Disabled internal auto-connect in favor of external startup script!
                    available_inputs++;
                } else {
                    // Remove JACK client input ports that have no corresponding soundcard audio input (capture) port.
    ...
    // 10. Connect the output ports of the JACK client with the audio outputs of the soundcard (JACK calls them "input ports", because they are writable by the JACK client).
    if(running) {
        const char** ports = jack_get_ports(jackClient, NULL, NULL, JackPortIsPhysical | JackPortIsInput);

        if(ports == NULL) {
            running = false;
            std::cerr << "[JACK][ERROR]: No physical audio output (playback) ports are available!\n";
        }

        // Proceed only if the soundcard has physical audio output (playback) ports and take care of the usable loudspeakers!
        if(running) {
            for(size_t i = 0; i < speakers->size(); ++i) {
                if(!((jack_port_name(jackOutputs[i]) == NULL) || (ports[i] == NULL))) { // Do not connect to NULL ports
                    //jack_connect(jackClient, jack_port_name(jackOutputs[i]), ports[i]); // TODO: Disabled internal auto-connect in favor of external startup script!
                    available_outputs++;
                } else {
                    // Remove the JACK client output port that can not be connected to the soundcard audio output (playback) port.
    ...
    // 11. Everything is set up correctly, now print a final message with the parameters of the JACK client.
    if(running) {
        std::cout << "\n" << std::endl;

        // Correct the number of available sources (inputs).
        if(twonderConf->noSources != available_inputs) {
            std::cout << "[JACK][WARNING]: Total number of sources changed from " << twonderConf->noSources << " to " << available_inputs << " due to input limitations of the soundcard!" << std::endl;
        ...
        if(static_cast<int>(speakers->size()) != available_outputs) {
            std::cout << "[JACK][WARNING]: Total number of speakers changed from " << speakers->size() << " to " << available_outputs << " due to output limitations of the soundcard!" << std::endl;
            speakers->resize(available_outputs);
        }

        std::chrono::duration<float, std::milli> delay_msec(1.0 / jack_get_sample_rate(jackClient) * jack_get_buffer_size(jackClient) * 1000.0 * 2.0);

        std::cout
                << "\n\n"
                << "[JACK]: JACK sample rate: " << jack_get_sample_rate(jackClient) << " Hz\n"
                << "[JACK]: JACK buffer size: " << jack_get_buffer_size(jackClient) << " samples\n"
                << "[JACK]: Round-Trip delay: " << delay_msec.count() << " milliseconds\n"
                << std::endl;
    }
...
public:
    MoveCommand(Vector3D& v, int id = 0, TimeStamp timestamp = (float)0, int durationInSamples = 10240) : Command(timestamp) {
        destination = v;
        sourceId    = id;
...
public:
    AngleCommand(float af, int id = 0, TimeStamp timestamp = (float)0, int durationInSamples = 10240): Command(timestamp) {
        angle     = af;
        sourceId  = id;
        duration  = durationInSamples;
    }
...
public:
    ListenerMoveCommand(float x, float y, int id = 0, TimeStamp timestamp = (float)0) : Command(timestamp) {
        destination_x = x;
        destination_y = y;
        listenerId    = id;
    }
...
int oscSrcPositionHandler(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "osc-position: src=" << sourceId << " x=" << newX << " y=" << newY << " ts=" << time << " dur=" << duration << std::endl;
    }

    Vector3D newPos(newX, newY, newZ);

    MoveCommand* moveCmd = new MoveCommand(newPos, sourceId, TimeStamp((wonder_frames_t)(time * (float) twonderConf->sampleRate)), (int)(duration * twonderConf->sampleRate));
...
int oscSrcPositionHandler3D(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "osc-position: src=" << sourceId << " x=" << newX << " y=" << newY << " z=" << newZ << " ts=" << time << " dur=" << duration << std::endl;
    }
...
int oscSrcAngleHandler(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "osc-angle: src=" << sourceId << " a=" << newAngle << " ts=" << time << " dur=" << duration << std::endl;
    }
...
int oscSrcTypeHandler(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "osc-type-change: src=" << sourceId << " type=" << newType << " ts=" << time << std::endl;
    }
...
int oscSrcDopplerHandler(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "osc-doppler-change: src=" << sourceId << " doppler=" << useDoppler << " ts=" << time << std::endl;
    }
...
int oscReplyHandler(handlerArgs) {
    if(twonderConf->verbose) {
        std::cout << "[V-OSCServer] reply to: " << &argv[0]->s << " state=" << argv[1]->i << " msg=" << &argv[2]->s << std::endl;
    }
...
int oscListenerPosHandler(handlerArgs) {
    ...
    if(twonderConf->verbose) {
        std::cout << "listener-position: id=" << listenerId << " x=" << newX << " y=" << newY << std::endl;
    }
...
int oscNoSourcesHandler(handlerArgs) {
    // this handler is only allowed to be called once and only with a valid value at startup, so ignore further messages
...
int oscRenderPolygonHandler(handlerArgs) {
    ...
    for(int i = 1; i <= noPoints; ++i) {
        twonderConf->renderPolygon.push_back(Vector3D(argv[(i * 3) - 1]->f, argv[i * 3]->f, argv[(i * 3) + 1]->f));
    }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cerr << "could not read speaker configuration file " << twonderConf->speakersConfigfile << ", exiting now... " << std::endl;
        std::exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        std::cerr << "[twonder] Could not create server, maybe the server( using the same port ) is already running?" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    ...
    while(twonderConf->noSources == 0) {
        ...
        if(timeoutCounter == 5) {
            std::cerr << "could not establish connection to cwonder, exiting now... " << std::endl;
            std::exit(EXIT_FAILURE);
        }
