...
{
  public:
    FrameQueue() { pthread_mutex_init(&mutex, nullptr); }

    ~FrameQueue() { pthread_mutex_destroy(&mutex); }
...
int startDaemon() {
    ...
    if (!fin.is_open()) {
        ...
        // CAUTION: anyone is able to create files, but is not able to delete anyone elses
        // files but their own
...
int cleanDaemon() {
    ...
    if (jfConf->daemon) {
        if (remove(path_pidfile.c_str()) != 0) {
            ...
            if (jfConf->verbose) fmt::print("[V-Config] Deleted pid file.\n");
        }
    }
    return 0;
}

int genericHandler(const char* path, const char* types, lo_arg** argv, int argc,
                   lo_message data, void* user_data) {
    int i;

    fmt::print(
        ("[V-OSCServer] --- generic handler --- No handler implemented for message: "
         + (string)path)
            .c_str());

    for (i = 0; i < argc; ++i) cout << "arg " << i << "'" << types[i] << "'" << endl;
    cout << endl;

    lo_address from = lo_message_get_source(data);
    lo_send(from, "/WONDER/reply", "sis", path, 1,
            "[genericHandler] No handler implemented for message");
    return 0;
}

int connectHandler(const char* path, const char* types, lo_arg** argv, int argc,
                   lo_message data, void* user_data) {
    start = true;
...
void showTime() {
    ...
    if (frameTime) {
        // We cast the unsigned int to a signed integer because OSC has no native support
        // for unsigned integers. When receiving we cast back to an uint.

        lo_send(jfConf->cwonderAddr, "/WONDER/jfwonder/frametime", "i",
                (int32_t)frameTime);

        if (jfConf->jfverbose)
            cout << "@ " << frameTime << " - " << lastFrame2 << " -> "
                 << (frameTime - lastFrame2) << endl;
...
void jackShutdown(void* arg) {
    lo_send(jfConf->cwonderAddr, "/WONDER/jfwonder/error", "s",
...
int main(int argc, char* argv[]) {
    ...
    fmt::print("[V-settings] cwonder host={}, port={}\n",
               lo_address_get_hostname(jfConf->cwonderAddr),
               lo_address_get_port(jfConf->cwonderAddr));
    ...
    if (loServer) {
        ...
        lo_server_thread_add_method(loServer, "/WONDER/jfwonder/connect", "",
