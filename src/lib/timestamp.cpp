...
void TimeStamp::addsec(float sec) {
    ...
    if(samples >= WONDER_MAX_FRAMES / 2) {
        std::cout << std::endl << "[V-TimeStamp::addsamp] The number of samples (" << samples << ") is bigger than WONDER_MAX_FRAMES/2" << std::endl;
    } else {
        time += samples;
    }
}
...
void TimeStampSc::update(wonder_frames_t nowTime) {
    ...
    if(nowTime < lastTime) {
        std::cout << std::endl << "[V-TimeStampSc::update] throw away nowtime=" << nowTime << " lasttime=" << lastTime << std::endl;
    } else {
        ...
        if(tmpper != period) {
            std::cout << std::endl << "[V-TimeStampSc::update] Out of sync: " << tmpper << std::endl;
        }
...
void TimeStampSc::show(char* name) {
    std::cout << name << "  time ( in samples ): " << time << "  in seconds: " << (float) time / sampleRate << "  lasttime: " << lastTime << " wraps: " << wraps << std::endl;
}
