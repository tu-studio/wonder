...
int Cwonder::loadProject(string path) {
    ...
    try {
        ...
        returnString = "project=" + path + (" load error. Project contains too many sources for the current system setup.");
    ...
    for(int i = 0; i < (int) scenario->sourcesVector.size(); ++i) {
        ...
        if(scenario->sourcesVector[i].active) {
            ...
            for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                ...
                // send type
                lo_send(streamIter->address, "/WONDER/source/type", "ii", i, scenario->sourcesVector[i].type);

                // send position
                lo_send(streamIter->address, "/WONDER/source/position3D", "iffff", i,
                        scenario->sourcesVector[i].pos[0],
                        scenario->sourcesVector[i].pos[1],
                        scenario->sourcesVector[i].pos[2],
                        duration);

                // send angle
                lo_send(streamIter->address, "/WONDER/source/angle", "iff", i, scenario->sourcesVector[i].angle, duration);

                // send doppler effect
                lo_send(streamIter->address, "/WONDER/source/dopplerEffect", "ii", i, scenario->sourcesVector[i].dopplerEffect);
            }
            ...
            for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
                ...
                // send position
                lo_send(streamIter->address, "/WONDER/source/position", "iff", i, scenario->sourcesVector[i].pos[0], scenario->sourcesVector[i].pos[1]);

                // send angle
                lo_send(streamIter->address, "/WONDER/source/angle", "if", i, scenario->sourcesVector[i].angle);

                // send name
                lo_send(streamIter->address, "/WONDER/source/name", "is", i, scenario->sourcesVector[i].name.c_str());

                // send groupID
                lo_send(streamIter->address, "/WONDER/source/groupID", "ii", i, scenario->sourcesVector[i].groupID);
                ...
                lo_send(streamIter->address, "/WONDER/source/rotationDirection", "ii", i, scenario->sourcesVector[i].invertedRotationDirection);

                // send scaling direction
                lo_send(streamIter->address, "/WONDER/source/scalingDirection", "ii", i, scenario->sourcesVector[i].invertedScalingDirection);
            }
        } else {
...
int Cwonder::setSourceName(lo_address from, int sourceID, string name) {
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        // send to stream receivers (visual)
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            if(!issame(from, streamIter->address)) {
...
int Cwonder::setSourcePosition(lo_address from, int sourceID, float x, float y, float duration, float timestamp) {
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        if(timestamp == PLAYNOW) {
            ...
            event->timestamp = nowTime;
            event->from      = lo_address_new(lo_address_get_hostname(from), lo_address_get_port(from));
            event->timestamp.addsec(timestamp);
...
int Cwonder::setSourcePosition3D(lo_address from, int sourceID, float x, float y, float z, float duration, float timestamp) {
    if(!checkScenario()) {
        return 5;
    }
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        if(timestamp == PLAYNOW) {
            ...
            // send to stream receivers (render)
            for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                lo_send(streamIter->address, "/WONDER/source/position3D", "iffff", sourceID, x, y, z, duration);
            }
        ...
        st << "sourceID " << sourceID << ", position " << x << ", " << y << ", " << z << " ok.";
        returnString = st.str();
    } else {
        st << "sourceID " << sourceID << ", position " << x << ", " << y << ", " << z << " not ok. Invalid id.";
        returnString = st.str();
        return 1; // source does not exist
    }
...
int Cwonder::setSourceAngle(lo_address from, int sourceID, float angle, float duration, float timestamp) {
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        if(timestamp == PLAYNOW) {
            // send to stream receivers (render)
            for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                lo_send(streamIter->address, "/WONDER/source/angle", "iff", sourceID, angle, duration);
            }
...
int Cwonder::setSourceGroupID(lo_address from, int sourceID, int groupID) {
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        ...
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            if(!issame(from, streamIter->address)) {
                lo_send(streamIter->address, "/WONDER/source/groupID", "ii", sourceID, groupID);
            }
        }
...
int Cwonder::setSourceColor(lo_address from, int sourceID, int red, int green, int blue) {
    ...
    if(sourceID >= 0 && sourceID < (int) scenario->sourcesVector.size() && scenario->sourcesVector[sourceID].active) {
        ...
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            if(!issame(from, streamIter->address)) {
                lo_send(streamIter->address, "/WONDER/source/color", "iiii", sourceID, red, green, blue);
            }
        }
        ...
        st << "sourceID " << sourceID << ", color " << red << ", " << green << ", " << blue << " not ok. Invalid id.";
        returnString = st.str();
        return 1; // source does not exist
    }
...
int Cwonder::setListenerPosition(lo_address from, int listenerID, float x, float y) {
    ...
    if(listenerID >= 0) {
        ...
        // send to stream receivers (render)
        for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
            lo_send(streamIter->address, "/WONDER/listener/position", "iff", listenerID, x, y);
        }
        ...
        st << "listenerID " << listenerID << ", position " << x << ", " << y << " not ok. Invalid id.";
        returnString = st.str();
...
int Cwonder::setGroupColor(lo_address from, int groupID, int red, int green, int blue) {
    ...
    if(id >= 0  && id < (int) scenario->sourceGroupsVector.size() && scenario->sourceGroupsVector[ id ].active) {
        // send to stream receivers (visual)
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            if(!issame(from, streamIter->address)) {
                lo_send(streamIter->address, "/WONDER/group/color", "iiii", groupID, red, green, blue);
            }
        }
        ...
        st << "groupID " << groupID << ", color " << red << ", " << green << ", " << blue << " not ok." << " Invalid id.";
        returnString = st.str();
        return 1; // group does not exist
    }
        ...
        if(scenario) {
            for(int i = 0 ; i < (int) scenario->sourcesVector.size(); ++i) {
                ...
                if(source->active) {
                    ...
                    lo_send(newClientAddress, "/WONDER/source/position3D",    "ifff", i, source->pos[0], source->pos[1], source->pos[2]);
    ...
    for(int i = 0; i < (int) timeLine.getsize(); ++i) {
        ...
        if(ev == NULL) {
            ...
            if(ev->oscpath == "/WONDER/source/position") {
                // send to stream receivers (render)
                for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                    lo_send(streamIter->address, "/WONDER/source/position", "ifff", ev->id, ev->pos[0], ev->pos[1], ev->duration);
                }

                // send to stream receivers (visual)
                for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
                    if(!issame(ev->from, streamIter->address)) {
                        lo_send(streamIter->address, "/WONDER/source/position", "iff", ev->id, ev->pos[0], ev->pos[1]);
                    }
                }
                ...
                // send to stream receivers (render)
                for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                    lo_send(streamIter->address, "/WONDER/source/position3D", "iffff", ev->id, ev->pos[0], ev->pos[1], ev->pos[2], ev->duration);
                }
                ...
                // send to stream receivers (render)
                for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
                    lo_send(streamIter->address, "/WONDER/source/angle", "iff", ev->id, ev->angle, ev->duration);
                }
    ...
    for(int i = 0; i < (int) scenario->sourcesVector.size(); ++i) {
        ...
        for(streamIter = renderStream->begin(); streamIter != renderStream->end(); ++streamIter) {
            ...
            if(source->active) {
                ...
                // send position
                lo_send(address, "/WONDER/source/position3D", "ifff", i, source->pos[0], source->pos[1], source->pos[2]);
        ...
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            ...
            if(source->active) {
                ...
                // send color
                lo_send(address, "/WONDER/source/color", "iiii", i, source->color[0], source->color[1], source->color[2]);
    ...
    for(int i = 0; i < (int) scenario->sourceGroupsVector.size(); ++i) {
        ...
        for(streamIter = visualStream->begin(); streamIter != visualStream->end(); ++streamIter) {
            ...
            if(group->active) {
                ...
                // send position
                lo_send(address, "/WONDER/group/position", "iff", id, group->pos[0], group->pos[1]);

                // send color
                lo_send(address, "/WONDER/group/color", "iiii", id, group->color[0], group->color[1], group->color[2]);
            } else {
