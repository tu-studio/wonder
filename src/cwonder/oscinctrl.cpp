...
void OSCControl::addMethods() {
    ...
    // depending on mode ( basic or normal ) assign the appropriate handlers for incoming OSC messages
    if(cwonderConf->basicMode) {
        ...
        addMethod("/WONDER/source/position3D", "ifff",   forwardToRenderStreamHandler, this);
        addMethod("/WONDER/source/position3D", "iffff",  forwardToRenderStreamHandler, this);
        addMethod("/WONDER/source/position3D", "ifffff", forwardToRenderStreamHandler, this);
...
int OSCControl::replyHandler(handlerArgs) {
    ...
    log << "[V-OSCServer] reply to: " << &argv[0]->s << " state=" <<  argv[1]->i << " msg=" << &argv[2]->s;
...
int OSCControl::sourcePosition3DHandler(handlerArgs) {
    ...
    int ret = cwonder->setSourcePosition3D(from, argv[0]->i, argv[1]->f, argv[2]->f, argv[3]->f, duration, time);
...
int OSCControl::sourceColorHandler(handlerArgs) {
    ...
    int ret = cwonder->setSourceColor(from, argv[0]->i, argv[1]->i, argv[2]->i, argv[3]->i);
...
int OSCControl::groupColorHandler(handlerArgs) {
    ...
    int ret = cwonder->setGroupColor(from, argv[0]->i, argv[1]->i, argv[2]->i, argv[3]->i);
