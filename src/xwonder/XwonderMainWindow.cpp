...
XwonderMainWindow::XwonderMainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    if (xwConf->runWithoutCwonder) { basicWindowTitle.prepend("DEMOMODE --- "); }
    ...
    connect(scrollbar, SIGNAL(valueChanged(int)), channelsView, SLOT(updateWrapped(int)));
    ...
    // init sources3DWidget and ChannelsWidget if we now the maximum number of sources
    // allowed
    connect(oscReceiver, SIGNAL(maxNoSourcesWasSet()), channelsView, SLOT(init()));
    connect(oscReceiver, SIGNAL(maxNoSourcesWasSet()), sources3DView, SLOT(init()));

    // for demo mode only
    if (xwConf->runWithoutCwonder) {
        connect(oscReceiver, SIGNAL(maxNoSourcesWasSet()), this, SLOT(setDemoMode()));
    }

    // connections for all events of the Source3DWidget which shall produce an OSC message
    connect(sources3DView, SIGNAL(sourcePositionChanged(const Source&)), this,
            SLOT(sendOSCSourcePosition(const Source&)));
    connect(sources3DView, SIGNAL(sourceOrientationChanged(const Source&)), this,
            SLOT(sendOSCSourceOrientation(const Source&)));
    connect(sources3DView, SIGNAL(sourceColorChanged(int, const QColor&)), this,
            SLOT(sendOSCSourceColor(int, const QColor&)));
    connect(sources3DView, SIGNAL(sourceGroupIDChanged(int, int)), this,
            SLOT(sendOSCSourceGroupID(int, int)));
    connect(sources3DView, SIGNAL(groupActivated(int)), this,
            SLOT(sendOSCGroupActivate(int)));
    connect(sources3DView, SIGNAL(groupDeactivated(int)), this,
            SLOT(sendOSCGroupDeactivate(int)));
    connect(sources3DView, SIGNAL(groupPositionChanged(const SourceGroup&)), this,
            SLOT(sendOSCGroupPosition(const SourceGroup&)));
    connect(sources3DView, SIGNAL(groupColorChanged(int, const QColor&)), this,
            SLOT(sendOSCGroupColor(int, const QColor&)));

    // snapshotSelector connections
    connect(snapshotSelector, SIGNAL(closedMyself()), this,
            SLOT(snapshotSelectorClosedItself()));
    connect(snapshotSelector, SIGNAL(takeSnapshotSignal(unsigned int, QString)), this,
            SLOT(sendOSCSnapshotTake(unsigned int, QString)));
    connect(snapshotSelector, SIGNAL(deleteSnapshotSignal(unsigned int)), this,
            SLOT(sendOSCSnapshotDelete(unsigned int)));
    connect(snapshotSelector, SIGNAL(renameSnapshotSignal(unsigned int, QString)), this,
            SLOT(sendOSCSnapshotRename(unsigned int, QString)));
    ...
    connect(snapshotSelector, SIGNAL(recallSnapshotSignal(unsigned int, double)), this,
            SLOT(sendOSCSnapshotRecall(unsigned int, double)));

    // streamclientwidget connections
    connect(streamClientWidget, SIGNAL(closedMyself()), this,
            SLOT(streamClientWidgetClosedItself()));
    ...
    connect(snapshotSelector, SIGNAL(modified()), this, SLOT(setModified()));
    // connect( sources3DView, SIGNAL( modified() ),
    // this,          SLOT  ( setModified() ) );

    // sources3DWidget connections (more are made, when the ChannelsWidet is created)
    connect(sources3DView, SIGNAL(displaySourceCoordinates(const Source&)), this,
            SLOT(updateStatusBar(const Source&)));
    connect(sources3DView, SIGNAL(displayGroupCoordinates(const SourceGroup&)), this,
            SLOT(updateStatusBar(const SourceGroup&)));

    // connections to the oscReceiver
    connect(oscReceiver, SIGNAL(sourceActivated(int)), this, SLOT(activateSource(int)));
    connect(oscReceiver, SIGNAL(sourceDeactivated(int)), this,
            SLOT(deactivateSource(int)));
    connect(oscReceiver, SIGNAL(sourcePositionChanged(int, float, float)), sources3DView,
            SLOT(changeSourcePosition(int, float, float)));
    connect(oscReceiver, SIGNAL(sourceOrientationChanged(int, float)), sources3DView,
            SLOT(changeSourceOrientation(int, float)));
    connect(oscReceiver, SIGNAL(sourceTypeChanged(int, bool)), sources3DView,
            SLOT(changeSourceType(int, bool)));
    connect(oscReceiver, SIGNAL(sourceNameChanged(int, QString)), sources3DView,
            SLOT(changeSourceName(int, QString)));
    connect(oscReceiver, SIGNAL(sourceColorChanged(int, QColor)), sources3DView,
            SLOT(changeSourceColor(int, QColor)));
    connect(oscReceiver, SIGNAL(sourceGroupIDChanged(int, int)), sources3DView,
            SLOT(changeSourceGroupID(int, int)));
    connect(oscReceiver, SIGNAL(sourceRotationDirectionChanged(int, bool)), sources3DView,
            SLOT(changeSourceRotationDirection(int, bool)));
    connect(oscReceiver, SIGNAL(sourceScalingDirectionChanged(int, bool)), sources3DView,
            SLOT(changeSourceScalingDirection(int, bool)));
    connect(oscReceiver, SIGNAL(sourceRecordModeChanged(int, bool, bool)), sources3DView,
            SLOT(changeSourceRecordMode(int, bool, bool)));
    connect(oscReceiver, SIGNAL(sourceReadModeChanged(int, bool, bool)), sources3DView,
            SLOT(changeSourceReadMode(int, bool, bool)));
    connect(oscReceiver, SIGNAL(groupActivated(int, bool)), sources3DView,
            SLOT(activateGroup(int, bool)));
    connect(oscReceiver, SIGNAL(groupDeactivated(int, bool)), sources3DView,
            SLOT(deactivateGroup(int, bool)));
    connect(oscReceiver, SIGNAL(groupPositionChanged(int, float, float)), sources3DView,
            SLOT(changeGroupPosition(int, float, float)));
    ...
    connect(oscReceiver, SIGNAL(renderPolygonReceived()), sources3DView,
            SLOT(updateRoompolygon()));

    connect(oscReceiver, SIGNAL(timeChanged(int, int, int, int)), timeLCD,
            SLOT(setTime(int, int, int, int)));
    connect(oscReceiver, SIGNAL(projectXMLDump(QString)), this,
            SLOT(fetchOSCProjectXMLDump(QString)));
    connect(oscReceiver, SIGNAL(scoreplayerStopped()), this, SLOT(scoreplayerStopped()));
    connect(oscReceiver, SIGNAL(scoreplayerStarted()), this, SLOT(scoreplayerStarted()));
    connect(oscReceiver, SIGNAL(scoreplayerRecordModeChanged(bool, bool)), this,
            SLOT(setRecordmodeInScoreplayer(bool, bool)));
    connect(oscReceiver, SIGNAL(scoreplayerReadModeChanged(bool, bool)), this,
            SLOT(setReadmodeInScoreplayer(bool, bool)));
    connect(oscReceiver, SIGNAL(mmcChanged(bool, bool)), this,
            SLOT(enableMMC(bool, bool)));
    connect(oscReceiver, SIGNAL(msrcChanged(bool, bool)), this,
            SLOT(enableMSRC(bool, bool)));
    connect(oscReceiver, SIGNAL(OSCErrorMessage(QString)), this,
            SLOT(fetchOSCErrorMessage(QString)));
...
QSize XwonderMainWindow::minimumSizeHint() const { return QSize(512, 384); }
...
void XwonderMainWindow::keyPressEvent(QKeyEvent* event) {
    ...
    case Qt::Key_R:  // toogle recordMode of current source or group
        if (!projectOnlyMode) { QApplication::sendEvent(sources3DView, event); }
...
void XwonderMainWindow::sendOSCSourcePosition(const Source& source) {
    ...
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/position", "iff", source.getID(),
                coord.x, coord.y);
...
void XwonderMainWindow::sendOSCSourceOrientation(const Source& source) {
    ...
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/angle", "if", source.getID(),
                coord.orientation);
...
void XwonderMainWindow::sendOSCSourceGroupID(int sourceID, int groupID) {
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/groupID", "ii", sourceID, groupID);
...
void XwonderMainWindow::sendOSCSourceColor(int sourceID, const QColor& newColor) {
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/color", "iiii", sourceID,
...
void XwonderMainWindow::sendOSCSourceColor(unsigned int xID, const QColor& color) {
    ...
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/color", "iiii", source.getID(),
...
void XwonderMainWindow::sendOSCSourceName(unsigned int xID, const QString& name) {
    if (!sources3DView->xIDIsUsed(xID)) { return; }

    const Source& source = sources3DView->getSource(xID);

    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/source/name", "is", source.getID(),
...
void XwonderMainWindow::sendOSCSourceDopplerEffetChanged(unsigned int xID,
                                                         bool dopplerOn) {
    int useDoppler       = (int)dopplerOn;
    const Source& source = sources3DView->getSource(xID);

    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/source/dopplerEffect", "ii", source.getID(),
                useDoppler);
    }
...
void XwonderMainWindow::sendOSCGroupColor(int groupID, const QColor& newColor) {
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/group/color", "iiii", groupID,
                newColor.red(), newColor.green(), newColor.blue());
...
void XwonderMainWindow::sendOSCGroupPosition(const SourceGroup& group) {
    ...
    if (!xwConf->runWithoutCwonder)
        lo_send(xwConf->cwonderAddr, "/WONDER/group/position", "iff", group.getGroupID(),
                coord.x, coord.y);
...
void XwonderMainWindow::sendOSCNotifyIDChange(unsigned int xID, int oldID, int newID) {
    ...
    // send name, position, angle and type for xID, which has changed sourceID from oldID
    // to newID
    if (!xwConf->runWithoutCwonder) {
        SourceCoordinates coords =
            source.getCoordinatesRounded().mapGLCoordToWonderCoord();
        int sourceID = source.getID();
        int type     = (int)!source.isPlanewave();

        lo_send(xwConf->cwonderAddr, "/WONDER/source/activate", "i", newID);
        lo_send(xwConf->cwonderAddr, "/WONDER/source/name", "is", sourceID,
                (source.getName().toStdString().c_str()));
        lo_send(xwConf->cwonderAddr, "/WONDER/source/type", "ii", sourceID, type);
        lo_send(xwConf->cwonderAddr, "/WONDER/source/position", "iff", sourceID, coords.x,
                coords.y);
        lo_send(xwConf->cwonderAddr, "/WONDER/source/angle", "if", sourceID,
                coords.orientation);

        lo_send(xwConf->cwonderAddr, "/WONDER/source/groupID", "ii", sourceID,
                source.getGroupID());
        lo_send(xwConf->cwonderAddr, "/WONDER/source/rotationDirection", "ii", sourceID,
                (int)source.hasInvertedRotation());
        lo_send(xwConf->cwonderAddr, "/WONDER/source/scalingDirection", "ii", sourceID,
                (int)source.hasInvertedScaling());
        lo_send(xwConf->cwonderAddr, "/WONDER/source/dopplerEffect", "ii", sourceID,
                (int)source.hasDopplerEffect());
        lo_send(xwConf->cwonderAddr, "/WONDER/score/source/enableRecord", "ii", sourceID,
                (int)source.isRecordEnabled());
        lo_send(xwConf->cwonderAddr, "/WONDER/score/source/enableRead", "ii", sourceID,
                (int)source.isReadEnabled());
    ...
    // if it was a sourceID swap then send data for source which was changed from newID to
    // oldID
    if (swap) {
        if (!xwConf->runWithoutCwonder) {
            ...
            SourceCoordinates coords =
                swapped.getCoordinatesRounded().mapGLCoordToWonderCoord();
            int sourceID = oldID;
            int type     = (int)!swapped.isPlanewave();

            lo_send(xwConf->cwonderAddr, "/WONDER/source/name", "is", sourceID,
                    (swapped.getName().toStdString().c_str()));
            lo_send(xwConf->cwonderAddr, "/WONDER/source/type", "ii", sourceID, type);
            lo_send(xwConf->cwonderAddr, "/WONDER/source/position", "iff", sourceID,
                    coords.x, coords.y);
            lo_send(xwConf->cwonderAddr, "/WONDER/source/angle", "if", sourceID,
                    coords.orientation);
...
void XwonderMainWindow::setFilename() { projectname = filenameDialog->getFilename(); }

void XwonderMainWindow::toggleAddChannelActs(unsigned int notUsed1, int notUsed2,
                                             bool notUsed3) {
...
void XwonderMainWindow::setProjectWithScoreMode() {
    ...
    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/stream/score/send", "s",
                "/WONDER/score/status");
    }
}
...
void XwonderMainWindow::createActions() {
    ...
    setRecordmodeInScoreplayerAct =
        new QAction(QIcon(":/record_icon"), "Record mode", this);
    setReadmodeInScoreplayerAct = new QAction(QIcon(":/read_icon"), "Read mode", this);
    returnToZeroInScoreplayerAct =
        new QAction(QIcon(":/RTZ_icon"), "Return to zero", this);
    ...
    connect(newProjectWithScoreAct, SIGNAL(triggered()), this,
            SLOT(newProjectWithScore()));
    connect(newProjectWithoutScoreAct, SIGNAL(triggered()), this,
            SLOT(newProjectWithoutScore()));
    connect(openProjectAct, SIGNAL(triggered()), this, SLOT(openProject()));
    connect(saveAct, SIGNAL(triggered()), this, SLOT(save()));
    connect(saveAsAct, SIGNAL(triggered()), this, SLOT(saveAs()));
    connect(exitAct, SIGNAL(triggered()), this, SLOT(close()));

    connect(resetViewAct, SIGNAL(triggered()), sources3DView, SLOT(resetView()));
    connect(lockViewAct, SIGNAL(toggled(bool)), sources3DView, SLOT(lockView(bool)));
    connect(lockClickAct, SIGNAL(toggled(bool)), sources3DView, SLOT(lockClick(bool)));
    connect(showPerspectiveViewAct, SIGNAL(toggled(bool)), sources3DView,
            SLOT(showPerspectiveView(bool)));
    connect(showCoordOriginAct, SIGNAL(toggled(bool)), sources3DView,
            SLOT(showOrigin(bool)));
    connect(showNamesAct, SIGNAL(toggled(bool)), sources3DView, SLOT(showNames(bool)));
    connect(showCoordinatesAct, SIGNAL(toggled(bool)), sources3DView,
            SLOT(showCoordinates(bool)));
    connect(showSnapshotSelectorAct, SIGNAL(toggled(bool)), this,
            SLOT(showSnapshotSelector(bool)));
    connect(showStreamClientWidgetAct, SIGNAL(toggled(bool)), this,
            SLOT(showStreamClientWidget(bool)));
    connect(takeSnapshotAct, SIGNAL(triggered()), snapshotSelector, SLOT(takeSnapshot()));
    connect(enableMMCAct, SIGNAL(triggered(bool)), this, SLOT(enableMMC(bool)));
    connect(enableMSRCAct, SIGNAL(triggered(bool)), this, SLOT(enableMSRC(bool)));
    connect(startScoreplayerAct, SIGNAL(triggered()), this, SLOT(startScoreplayer()));
    connect(stopScoreplayerAct, SIGNAL(triggered()), this, SLOT(stopScoreplayer()));
    connect(setRecordmodeInScoreplayerAct, SIGNAL(triggered(bool)), this,
            SLOT(setRecordmodeInScoreplayer(bool)));
    connect(setReadmodeInScoreplayerAct, SIGNAL(triggered(bool)), this,
            SLOT(setReadmodeInScoreplayer(bool)));
    connect(returnToZeroInScoreplayerAct, SIGNAL(triggered()), this,
            SLOT(returnToZeroInScoreplayer()));
    connect(clearScoreplayerAct, SIGNAL(triggered()), this,
            SLOT(fetchClearScoreRequest()));
    connect(setStartScenarioInScoreplayerAct, SIGNAL(triggered()), this,
            SLOT(setStartScenarioInScoreplayer()));
    ...
    if (fetchUnsavedChanges()) {
        ...
        while (!okToProceed) {
            ...
            if (!localMode) {
                ...
                if (dialogReturnCode == QDialog::Rejected) { return; }

                if (!projectname.isEmpty()) { okToProceed = true; }
            } else {
                QMessageBox::information(
                    this, "Xwonder",
                    "Please select the file in which your Project"
                    " should be stored.\nYou may also enter a new name for a "
                    " non-existing file.");
                projectname =
                    QFileDialog::getSaveFileName(this, "File for new Project:", ".");
                ...
                // because cwonder does not accept /WONDER/project/create with existing
                // files
...
void XwonderMainWindow::openProject() {
    ...
    if (fetchUnsavedChanges()) {
        if (!localMode) {
            ...
            projectname = QFileDialog::getOpenFileName(this, "Open File", ".",
                                                       "WONDER projectfiles (*.xml)");
        }
        ...
        if (!projectname.isEmpty()) {
            ...
            if (!xwConf->runWithoutCwonder) {
                lo_send(xwConf->cwonderAddr, "/WONDER/project/load", "s",
                        projectname.toStdString().c_str());
            }
...
void XwonderMainWindow::fetchOSCProjectXMLDump(QString xmlProject) {
    if (!myProjectLoadRequest) {
        ...
        int answer = QMessageBox::warning(this, "XWonder - Project loaded/created",
                                          "A Project has been loaded or created!\n"
...
//{
//      //TEST
//      //lo_send( xwConf->cwonderAddr, "/WONDER/stream/visual/send", "siiii",
//      "/WONDER/xwonder/mtctime",0,13,2,40); if(showIt)
//              timeSelector->show();
//      else
//              timeSelector->hide();
//}
...
//
//    if( ! xwConf->runWithoutCwonder )
//        lo_send( xwConf->cwonderAddr, "/WONDER/score/load", "s",
//        fullFilename.toStdString().c_str() );
//
//    setNotModified();
//    return;
...
bool XwonderMainWindow::fetchUnsavedChanges() {
    ...
    if (xwConf->runWithoutCwonder) { return true; }
...
bool XwonderMainWindow::saveAs() {
    ...
    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/project/save", "s",
...
void XwonderMainWindow::setNotModified() {
    ...
    if (!projectname.isEmpty()) { title += (" --- " + projectname); }
...
void XwonderMainWindow::updateStatusBar(const Source& source) {
    ...
    if (source.isPlanewave()) {
        message +=
            ", Orientation (in deg): " + QString::number(coord.orientation, 'f', 1);
    }
...
void XwonderMainWindow::sendOSCSnapshotTake(unsigned int snapshotID, QString name) {
    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/snapshot/take", "is", snapshotID,
                name.toStdString().c_str());
    }
...
void XwonderMainWindow::sendOSCSnapshotRecall(unsigned int snapshotID,
                                              double transitionTime) {
    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/snapshot/recall", "if", snapshotID,
...
void XwonderMainWindow::sendOSCSnapshotRename(unsigned int snapshotID, QString newName) {
    if (!xwConf->runWithoutCwonder) {
        lo_send(xwConf->cwonderAddr, "/WONDER/snapshot/rename", "is", snapshotID,
...
void XwonderMainWindow::parseLoadedProjectXMLDump(QString xmlProject) {
    ...
    // get the projectname
    if (!myProjectLoadRequest) { projectname = projectRoot.attribute("name"); }
    ...
    for (; !sources.isNull(); sources = sources.nextSiblingElement("source")) {
        ...
        for (int i = 0; i < attributes.length(); ++i) {
            ...
            if (attr.name() == "id") {
                ...
                if (attr.value().toInt() == 0) { planewave = true; }
